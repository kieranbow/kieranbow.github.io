...
!function(e)
{
	...
	e.fn.poptrox=function(o)
	{
		...
		var p, i, s = e.extend(
		{
				...
				onPopupClose:null,onPopupOpen:null},o),
				r = e(this),
				n = e("body"),
				...
				m.length > 0 && (x.css("padding-bottom",s.popupCaptionHeight+"px"),m.css("position","absolute").css("left","0").css("bottom","0").css("width","100%").css("text-align","center").css("height",s.popupCaptionHeight+"px").css("line-height",s.popupCaptionHeight+"px"),s.popupCaptionTextSize&&m.css("font-size",popupCaptionTextSize)),
				...
				}).on("stopSpinning",function(e)
				{
					...
					return l.on("resize orientationchange",function(){f()}),m.on("update",function(e,o)
					{
					...
					C.css("cursor","pointer").on("click",function(e)
					{
						...
						y.on("click",function(e)
						{
							...
							a.css("position","fixed").css("left",0).css("top",0).css("z-index",s.baseZIndex).css("width","100%").css("height","100%").css("text-align","center").css("cursor","pointer").appendTo(s.parent).prepend('<div style="display:inline-block;height:100%;vertical-align:middle;"></div>').append('<div style="position:absolute;left:0;top:0;width:100%;height:100%;background:'+s.overlayColor+";opacity:"+s.overlayOpacity+";filter:alpha(opacity="+100*s.overlayOpacity+');"></div>').hide().on("touchmove",function(e){return!1}).on("click",function(e)
							{
								e.preventDefault(),
								e.stopPropagation(),
								...
								{
									var e=d+1;e>=u.length&&(e=0),x.trigger("poptrox_switch",[e])}).on("poptrox_previous",function()
									{
										var e=d-1;e<0&&(e=u.length-1),x.trigger("poptrox_switch",[e])}).on("poptrox_reset",function()
										{
											f(),
											x.data("width",s.popupWidth).data("height",s.popupHeight),b.hide().trigger("stopSpinning"),
											m.hide(),
											C.hide(),
											P.hide(),
											v.hide(),
											w.attr("src","").detach()}).on("poptrox_open",function(e,o)
											{
												return !! h || (h = !0, 
											...
											}).on("poptrox_switch",function(o,t,p)
											{
												...
												if(!p && h) return !0;
												if(h =! 0, 
												x.addClass("loading").css("width",x.data("width")).css("height",x.data("height")),m.hide(),
												...
												}):w.attr("src",i.src),"image"!=i.type)
												{
													...
													w.css("position","relative").css("outline","0").css("z-index",s.baseZIndex+100).width(r).height(n)}b.trigger("startSpinning").fadeIn(300),x.show(),s.popupIsFixed?(x.removeClass("loading").width(s.popupWidth).height(s.popupHeight),w.on("load",function()
													{
														w.off("load"),b.hide().trigger("stopSpinning"),m.trigger("update",[i.captionText]).fadeIn(s.fadeSpeed),C.fadeIn(s.fadeSpeed),v.css("text-indent",0).hide().fadeIn(s.fadeSpeed,function(){h=!1}),d=t,P.fadeIn(s.fadeSpeed)})):w.on("load",function() 
												{
												...
												}), "image" != i.type && w.trigger("load")}).on("poptrox_close",function() 
								{
