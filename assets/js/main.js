...
(function($) {
	...
	if (browser.canUse('transition')) 
	{
		...
		$window.on('resize', function() 
		{
			...
			resizeTimeout = window.setTimeout(function() { $body.removeClass('is-resizing'); }, 100);
		});
	}
	...
	// Closer.
	$closer.on('click', function(event) { $this.trigger('---hide'); });

	// Events.
	$this.on('click', function(event) { event.stopPropagation(); }).on('---toggle', function() 
	{
	...
	$main.poptrox(
	{
		...
		overlayOpacity: 0.9, // <-- Change this to change overlay opacity
		popupCloserText: '',
		popupHeight: 150,
		popupLoaderText: '',
		popupSpeed: 300,
